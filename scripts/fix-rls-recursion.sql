...
-- 2. DOCUMENTS: Any authenticated user can read/write
...
CREATE POLICY "documents_select_all_auth" ON documents FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "documents_insert_auth" ON documents FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "documents_update_auth" ON documents FOR UPDATE USING (auth.role() = 'authenticated');
CREATE POLICY "documents_delete_auth" ON documents FOR DELETE USING (auth.role() = 'authenticated');

-- 3. DELIVERY LOGS: Any authenticated user can read/write
...
CREATE POLICY "delivery_select_all_auth" ON delivery_logs FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "delivery_insert_auth" ON delivery_logs FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "delivery_update_auth" ON delivery_logs FOR UPDATE USING (auth.role() = 'authenticated');
CREATE POLICY "delivery_delete_auth" ON delivery_logs FOR DELETE USING (auth.role() = 'authenticated');
